...
**Important — $l(y)$ and $q(y)$ are the *net* running loads (aerodynamic +
inertial).** The solver only sees these two distributions, so each must already
include **every** vertical force and pitching moment on the wing, not just the
air load. At a load factor $n$ (e.g. $n=2.5$ in a limit manoeuvre, $n=1$ in level
flight) the vertical running load is the aerodynamic lift **minus** the $n$-times
weight of the wing structure, the fuel in the tanks, and any engines / pylons /
external stores / landing gear:

$$
l(y) = l_{\text{aero}}(y) \;-\; n g \,\big[\, m_{\text{wing}}(y) + m_{\text{fuel}}(y) \,\big] \;-\; n g \sum_k w_k(y),
$$

where the $m(y)$ are distributed masses per unit span and the $w_k(y)$ are
concentrated weights (engine, pylon, tip tank, gear) **smeared over the span they
occupy**. The distributed masses give **inertial relief** that offloads the lift;
in a hard pull-up the net load can even reverse sign inboard.

The torsion $q(y)$ likewise sums **all** pitching moments about the elastic axis:
the aerodynamic moment $l_{\text{aero}}\,e$ **plus** each weight acting at its own
chordwise offset $d$ from the elastic axis (fuel CG, pylon/engine, tip tank),
$q(y) = \sum(\text{force})\times(\text{arm})$. Assembling these balanced
distributions is the analyst's job upstream of WingBox — the solver integrates
whatever $l, q$ you give it.
...
![Wing section: shear centre, bending axes, torsion cell and shear webs](docs/wing_section.svg)

### Shear centre (SC) / elastic axis
The point that matters here is the **shear centre**: the point of the section
where a transverse load causes **bending without twist**. Push the wing there
and it just bends; push it anywhere else and it also rotates. The line of shear
centres along the span is the **elastic axis** — the axis the wing twists about.
WingBox places its **nodes on this axis** (the `SC` field in the model), so
bending and torsion decouple cleanly and the torsion input $q(y)$ is simply
lift × offset. (Strictly, the shear centre — for bending/torsion decoupling — and
the centroid / "tension centre" — for axial/bending decoupling — are *different*
points in general; for a doubly-symmetric section they coincide. `SC` should be
the shear centre. WingBox keeps **bending↔torsion decoupled** — valid for
isotropic, roughly symmetric metal sections, but **not** for the bend–twist
coupling of composite laminates — while the two **bending planes** *can* be
coupled through the product of inertia $I_{yz}$, see below.)
...
### Product of inertia $I_{yz}$  →  coupling of the two bending planes
$I_{yz}$ (units m⁴) is what makes flap and edge bending **talk to each other**.
It is **zero** when the local $(y,z)$ axes are the section's *principal* axes —
the case for a perfectly symmetric section. It is **non-zero** when those axes
are tilted relative to the principal ones, which happens for asymmetric sections
or, typically, when **streamwise sections are used on a swept wing**. The
constitutive law then couples the two bending moments,

$$
\begin{bmatrix} M_y \\ M_z \end{bmatrix}
= E\begin{bmatrix} I_y & I_{yz} \\ I_{yz} & I_z \end{bmatrix}
\begin{bmatrix} \kappa_y \\ \kappa_z \end{bmatrix},
$$

so a pure lift load (flap bending) also produces a **chordwise deflection**, and
vice-versa. In the bundled example a small $I_{yz}$ (≈ 7 % correlation) bends the
tip ~11 mm fore-and-aft even though every applied load is vertical. Leave
$I_{yz}=0$ (the default) for principal-axis sections. Note this is a
**bending↔bending** coupling; WingBox still keeps bending decoupled from torsion.
...
|--------|----------|----------------------|------------------|
| `SC` | shear centre position | — | axis of twist; sets the lift offset $e$ |
...
| $I_{yz}$ | product of inertia (default 0) | $EI_{yz}$ bend–bend coupling | flap↔edge coupling (swept/asymmetric) |
...
2. **Element stiffness.** For each element we build an $18\times18$ matrix
   ...
   nodal forces. The section properties are **interpolated to the integration
   points** from the element's three nodal sections, so a tapered element varies
   its stiffness continuously rather than using one representative section.
...
For a **uniform** beam (constant section, unswept) this is a case where 1-D
finite elements are **nodally exact** — the computed nodal values match the
textbook Timoshenko cantilever formulas to machine precision (see
[§8](#8-verification)). For a real **tapered, swept** wing that exactness no
longer holds strictly — the polynomial shape functions cannot reproduce the
variable-$EI$ solution exactly — but the section properties **are interpolated
to the Gauss points** from the three nodal sections (not held constant per
element), so a handful of elements is already well converged (the 6-element
example agrees with the equilibrium load diagrams of [§5](#5-quick-start) to a
fraction of a percent). The full derivation
...
coords = np.array([s["SC"] for s in load_stations("wings/pc24_wing_sections.json")])
...
The root values ($V(0)\approx100$ kN, $M(0)\approx354$ kN·m, $T(0)\approx15$
kN·m) are the total shear/moment/torque reacted at the fuselage. The shear and
bending moment match the `Solution` root reactions to within the FE
load-quadrature error (< 0.1 %); the torque agrees in magnitude but the reaction
*moment about the span* additionally carries a projection of the bending moment
because the wing is swept, so compare it with care.
...
A wing is a list of **stations** along the span. Each station gives the shear
centre `SC` (its 3-D position) and the section properties there. Properties may
...
```json
{
  "Wingsection": {
    "station1": {
      "SC": [0.0, 0.0, 0.0],
      "E": 71.0e9, "nu": 0.33,
      "A": 0.01716, "Iy": 0.00263177, "Iz": 0.00029243, "Iyz": 5.849e-05, "J": 0.00087008,
      "ky": 0.5, "kz": 0.5
    },
    "station2": { "SC": [0.062, 0.708, 0.0], "...": "..." },
    "station3": { "SC": [0.124, 1.417, 0.0], "...": "..." }
  }
}
```
...
**Reminder:** these `l` and `q` columns are the **net** running loads — the
aerodynamic distribution already combined with the ×$n$ weight of the wing
structure, fuel and any pylons/engines/stores/gear (see the note in §1). The
bundled example is *lift only* for clarity; a real load case would subtract the
inertial terms and add their torsion contributions.
...
```
Tip vertical deflection (global Z):  0.5003 m   (≈ 6% of semi-span)
Tip chordwise deflection (global X): +0.0110 m  (from Iyz coupling)
Tip twist (about global Y):          -0.0735 deg
Root reaction force  [N]:   [      0.        0.  -100064.6]
Root reaction moment [N.m]: [-354266.4   15984.0        0. ]
```

The vertical reaction (≈ 100 kN down) balances the applied lift, and the root
bending moment (≈ 354 kN·m) equals $\int l(y)\,y\,\mathrm{d}y$ — sanity checks a
flight-mechanics reader can verify by hand. The small **chordwise tip deflection
(≈ 11 mm)** is produced purely by the section product of inertia $I_{yz}$: every
applied load is vertical, yet the coupled bending planes bend the tip fore-and-aft.
(The reaction moment about the span, ≈ 16 kN·m, is *not* the pure torque
$T(0)\approx15$ kN·m: on a swept wing the root reaction about $Y$ mixes torsion
with a projection of the bending moment. The reactions themselves are unchanged
by $I_{yz}$ — they follow from equilibrium of the applied loads.)
...
$y$ and $z$ are the cross-section axes (the principal axes when $I_{yz}=0$). An
element has three nodes —
...
= \big[\, \varepsilon_x,\ \kappa_x,\ \kappa_y,\ \kappa_z,\ \gamma_{xy},\ \gamma_{xz} \,\big]^{\mathsf T}
= \big[\, u',\ r_x',\ r_y',\ r_z',\ v' - r_z,\ w' + r_y \,\big]^{\mathsf T}
$$

| strain | expression | meaning |
| --- | --- | --- |
| $\varepsilon_x$ | $u'$ | axial strain |
| $\kappa_x$ | $r_x'$ | torsional twist rate |
| $\kappa_y$ | $r_y'$ | bending curvature (local $x$–$z$ plane) |
| $\kappa_z$ | $r_z'$ | bending curvature (local $x$–$y$ plane) |
| $\gamma_{xy}$ | $v' - r_z$ | transverse shear, local $y$ |
| $\gamma_{xz}$ | $w' + r_y$ | transverse shear, local $z$ |
...
shear forces $V_y,V_z$ — follow the constitutive law
...
The only off-diagonal terms are the product of inertia $EI_{yz}$, which couples
the two bending moments $M_y, M_z$; they vanish ($I_{yz}=0$) when $(y,z)$ are the
section principal axes. Bending stays decoupled from axial, torsion and shear.

## A.5 Strong form (beam equilibrium)

With distributed loads $\mathbf{f} = [f_x, f_y, f_z, m_x, m_y, m_z]$ per unit
length, the six equilibrium equations are — in order — axial, torsion, then a
force/moment pair for bending in the $x$–$y$ plane and a pair for the $x$–$z$
plane:
...
with $\mathbf{D}_{\text{bend}} = \mathrm{diag}(EA, GJ, EI_y, EI_z, 0, 0)$ (plus
the off-diagonal $EI_{yz}$ coupling $M_y,M_z$ of §A.4) and
$\mathbf{D}_{\text{shear}} = \mathrm{diag}(0,0,0,0,GA_{sy},GA_{sz})$. The
product-of-inertia term lives in the bending group, so it is fully integrated.
This yields exactly **6 rigid-body modes and no spurious mechanism**.
...
(end nodes shared) and carries the three nodal `Section` objects; the
constitutive matrix $\mathbf D(\xi) = \sum_i N_i(\xi)\,\mathbf D_i$ is
interpolated to the Gauss points, so tapered properties vary within the element.
Element stiffnesses scatter into the global $6n\times6n$ matrix $\mathbf K$. The
distributed loads become a **consistent nodal load vector** through the shape
functions,
